...
def test_levels_four(fingers):
    print("\nTEST: niveles 0 -> 1 -> 2 -> 0 en 4 dedos")
    for level in [0, 1, 2, 0]:
        print(f"  set all 4 -> {level}")
        for f in fingers:
...
def main():
    ...
    print("Inicializando servos (4 dedos)...")
    ...
    try:
        ...
        for ch, levels in mapping.items():
            print(f"Letra {ch}: [Thumb, Index, Middle, Ring]={levels}")
